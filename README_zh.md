...
通过 TCP 远程执行 Dhizuku Device Owner 命令。
...
- **服务端**：所有 Android 架构（纯 Java 实现，无 native 依赖）
- **客户端（二进制）**：Android aarch64，无运行时依赖
- **客户端（Python）**：所有支持 Python 3.6+ 的平台

## 前提条件

- 已安装 Dhizuku 并正确激活为 Device Owner
- 已安装至少一个终端应用
...
dcli suspend com.example.app            # 挂起应用
dcli resume com.example.app             # 恢复挂起
dcli block_uninstall com.example.app    # 阻止卸载
dcli unblock_uninstall com.example.app  # 允许卸载
```
...
| unhide | <包名> | 恢复被隐藏的应用 |
| suspend | <包名> | 挂起指定应用（图标变灰） |
| resume | <包名> | 恢复被挂起的应用 |
| block_uninstall | <包名> | 阻止指定应用被卸载 |
| unblock_uninstall | <包名> | 允许指定应用被卸载 |
...
- 添加命令 suspend resume block_uninstall unblock_uninstall
- 添加英语支持
